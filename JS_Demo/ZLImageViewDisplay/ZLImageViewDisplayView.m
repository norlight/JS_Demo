...
//原库（见上）传入的参数是一个存放字符串的数组，改为存放UIImageView的
-(void) addImageviewsForMainScrollWithImages: (NSArray *) images{
    //设置ContentSize
    _mainScrollView.contentSize = CGSizeMake(_widthOfView * (images.count+1), _heightView);
    ...
    for ( int i = 0; i < _imageViewArray.count + 1; i ++) {
        ...
        if (i == 0) {
            //image = [_imageViewArray lastObject];
            UIImageView *tempImageView = [_imageViewArray lastObject];
            ...
            //image = _imageViewArray[i - 1];
            UIImageView *tempImageView = _imageViewArray[i - 1];
            tempImageView.frame = currentFrame;
